...
void physicsObject3D::newPosition(physicsObject3D x, physicsObject3D y, double rateOfEvolution) {
	...
	position = position + currentVelocity * rateOfEvolution + ((Vector3::normalize(x.position) * (x.mass / (x.position - position).magnitude() * (x.position - position).magnitude())) + (Vector3::normalize(y.position) * (y.mass / (y.position - position).magnitude() * (y.position - position).magnitude()))) * 0.00001 * rateOfEvolution * rateOfEvolution;
